 ...
 */
export function parseTreatmentFromOrder(order) {
 ...
 */
export async function getOrderTreatment(fastify, orderId) {
    ...
    try {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const fullOrder = await fastify.woo.get(`/orders/${orderId}`);
        return parseTreatmentFromOrder(fullOrder);
    }
    catch {
        return null;
    }
 ...
 */
export async function getOrdersWithTreatments(fastify, customerId, page = 1, perPage = 20) {
    const { orders } = await getCustomerOrders(fastify, customerId, page, perPage);
    // Fetch full orders with meta_data to parse treatments
    const ordersWithTreatments = await Promise.all(orders.map(async (order) => {
        try {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const fullOrder = await fastify.woo.get(`/orders/${order.id}`);
            const treatment = parseTreatmentFromOrder(fullOrder);
            return {
                order: {
                    id: order.id,
                    number: order.number,
