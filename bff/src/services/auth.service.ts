...
interface WordPressJWTResponse {
  token?: string;
  user?: {
    id: number;
    username: string;
...
export async function validateWordPressCredentials(
  ...
  try {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const data = await response.json() as WordPressJWTResponse;
 ...
 */
export async function getWooCommerceCustomerId(
  email: string,
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  wooClient: { get: (endpoint: string, params?: Record<string, string | number>) => Promise<any> }
): Promise<number | null> {
  try {
